    ...
    if (params.mergedBam == null) {
        if (params.sequencingPlatform.equalsIgnoreCase("PacBio")) {
            ...
            if (params.containsKey('ontReleaseData') && params.ontReleaseData) {
                ...
                qcFolder = NwgcONTCore.getReleaseSupQCFolder(ontDataFolder)
                ...
                LONGREAD_QC(ONT_RELEASE_BAMS.out.bam, ONT_RELEASE_BAMS.out.bai, 
                    params.sampleDirectory, qcFolder, fundamentalQCs)
                ...
                ONT_BASECALL()
                // FIXME: perform only the samstat [error rate] and coverage [mean cov>=25]
                //        need a separate version of LONGREAD_QC for SUP historical trending
                // TODO: adjust the folder
                ////FIMEX: testing purpose
                // all QCs: might be unnecessary
                // def fundamentalQCs = ['All']
                // all QCs: skip contamination until we handle insufficient coverage
                //          at least perform only the samstat [error rate] and coverage [mean cov>=25]
